...
int main(int argc, char * argv[])
{
  ...
  auto slow_square_client = node->create_client<SetBool>("slow_square");
  ...
  auto validate_client = node->create_client<SetBool>("validate");
  auto action_server = ctx.create_action_server<Fibonacci>(
    ...
    [&ctx, validate_client](
      ...
      for (int i = 2; i < goal.goal().order; i++) {
        ...
        RCLCPP_INFO(logger, "[fibonacci] feedback: seq[%d] = %d", i, feedback.sequence.back());
        co_await timer->next();
      }

      Fibonacci::Result result;
      result.sequence = feedback.sequence;
      RCLCPP_INFO(logger, "[fibonacci] done, length=%zu", result.sequence.size());
      co_return result;
    });
  ...
  auto client_task = ctx.create_task([&ctx]() -> rclcpp_async::Task<void> {
    ...
    Fibonacci::Goal goal;
    goal.order = 8;
    RCLCPP_INFO(logger, "[client] sending goal: order=%d", goal.order);
    auto goal_result = co_await ctx.send_goal<Fibonacci>(action_client, goal);
    if (!goal_result.ok()) {
      RCLCPP_ERROR(logger, "[client] goal failed: %s", goal_result.error_msg.c_str());
      co_return;
    }
    ...
    RCLCPP_INFO(logger, "[client] result: length=%zu, last=%d", seq.size(), seq.back());
    RCLCPP_INFO(logger, "[client] demo completed successfully!");
    RCLCPP_INFO(logger, "[client] no deadlock on single-threaded executor");
